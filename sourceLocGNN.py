...
graphType = 'graphon' # Type of graph: 'SBM', 'FacebookEgo', 'SmallWorld'
...
tMax = 20 # Maximum number of diffusion times (A^t for t < tMax)

nDataRealizations = 8 # Number of data realizations
nGraphRealizations = 1 # Number of graph realizations
nClasses = 10 # Number of source nodes to select

nNodes = 200 # Number of nodes
graphOptions = {} # Dictionary of options to pass to the createGraph function
nodesPooling = [100, 10]
otherGraphOptions = []
for i in range(len(nodesPooling)):
    otherGraphOptions.append(dict())
...
if graphType == 'SBM':
    ...
    if use234:
        nClasses = 2
elif graphType == 'graphon':
        # graphOptions['W'] = lambda y, x: math.exp(-2.3*(x - y)**2)
        graphOptions['W'] = lambda y, x: 0.5*(x * x+ y*y)
        graphOptions['Wtype'] = 'quadratic'
        graphOptions['labels'] = None
        graphOptions['sampling'] = False
        graphOptions['integration'] = True
        graphOptions['normalize'] = True
        graphOptions['irregular'] = True
        graphOptions['interval'] = np.sort( np.random.rand(nNodes))
        

        for i in range(len(nodesPooling)):
            otherGraphOptions[i]['W'] = graphOptions['W']
            otherGraphOptions[i]['labels'] = None          
            otherGraphOptions[i]['sampling'] = graphOptions['sampling']            
            otherGraphOptions[i]['integration'] = graphOptions['integration']
            otherGraphOptions[i]['normalize'] = graphOptions['normalize']
            otherGraphOptions[i]['irregular'] = graphOptions['irregular'] 
            otherGraphOptions[i]['normalize'] = graphOptions['normalize']  
            otherGraphOptions[i]['interval'] = np.sort( np.random.rand(nodesPooling[i]))
            # if i == 0:
            #     otherGraphOptions[i]['interval'] = np.sort( np.random.choice(graphOptions['interval'], nodesPooling[i], replace = False) )

            # else:
            #     otherGraphOptions[i]['interval'] = np.sort(np.random.choice(otherGraphOptions[i-1]['interval'], nodesPooling[i], replace = False))

#\\\ Save values:
writeVarValues(varsFile, {'nNodes': nNodes, 'graphType': graphType})
writeVarValues(varsFile, graphOptions)
writeVarValues(varsFile, otherGraphOptions[0])
...
if doGraphonGNN:
    ...
    modelGraphonGNN = {}
    modelGraphonGNN['name'] = 'GraphonGNN' # To be modified later on depending on the
        # specific ordering selected
    modelGraphonGNN['device'] = 'cuda:0' if (useGPU and torch.cuda.is_available()) \
                                     else 'cpu'
                                     
    #\\\ ARCHITECTURE
        
    # Select architectural nn.Module to use
    modelGraphonGNN['archit'] = archit.GraphonGNN
    # Graph convolutional layers
    modelGraphonGNN['dimNodeSignals'] = [1, 8, 8] # Number of features per layer
    modelGraphonGNN['nFilterTaps'] = [5, 5] # Number of filter taps
    modelGraphonGNN['bias'] = True # Include bias
    # Nonlinearity
    modelGraphonGNN['nonlinearity'] = nn.ReLU
    # Pooling
    modelGraphonGNN['poolingLabels'] = None # Number of nodes to keep
    # Readout layer
    modelGraphonGNN['dimLayersMLP'] = [nClasses]
    # Graph Structure
    modelGraphonGNN['GSOs'] = None # To be determined later on, based on data
    
    #\\\ TRAINER

    modelGraphonGNN['trainer'] = training.Trainer
    
    #\\\ EVALUATOR
    
    modelGraphonGNN['evaluator'] = evaluation.evaluate
    
        #\\\ Save Values:
    writeVarValues(varsFile, modelGraphonGNN)
    modelList += [modelGraphonGNN['name']]
...
if doSelectionGNN and doCoarsening:
    ...
    modelSelGNNcrs['poolingSize'] = [1, 1]
...
for graph in range(nGraphRealizations):
    ...
    if graphType != 'FacebookEgo':
        ...
        if graphType == 'graphon':
            Gs = [G]
        
            if graphOptions['labels'] == None and graphOptions['irregular'] == False:
                poolingLabels = [np.arange(0,1,1/G.N)]
            
            elif graphOptions['labels'] == None and graphOptions['irregular'] == True:
                poolingLabels = [graphOptions['interval']]
                

            else:
                poolingLabels = [graphOptions['labels']]
                
            for i in range(len(nodesPooling)):
                thisG = graphTools.Graph(graphType, nodesPooling[i], otherGraphOptions[i])
                thisG.computeGFT()
                if i < len(nodesPooling)-1:
                    Gs.append(thisG)
                if otherGraphOptions[i]['labels'] == None and graphOptions['irregular'] == False:
                    poolingLabels.append(np.arange(0,1,1/thisG.N))

                elif graphOptions['labels'] == None and graphOptions['irregular'] == True:
                    poolingLabels.append(otherGraphOptions[i]['interval'])
                    

                else:
                    poolingLabels.append(otherGraphOptions[i]['labels'])
    ...
    for realization in range(nDataRealizations):
        ...
        for thisModel in modelList:
            ...
            if 'crs' in thisModel:
                ...
                modelDict['GSO'] = S
            elif 'Graphon' in thisModel:
                modelDict['GSOs'] = [thisG.S.copy()/np.max(np.real(thisG.E)) for thisG in Gs]
                modelDict['poolingLabels'] = poolingLabels
            else:
