...
def createGraph(graphType, N, graphOptions):
    ...
    if graphType == 'SBM':
        ...
        sampling = graphOptions['sampling']
        irregular = graphOptions['irregular']
        integration = graphOptions['integration']
        normalize = graphOptions['normalize']
        ...
        if labels == None:
            ...
            for i in range(N):    
                for j in range(i,N):
                    ...
                    if integration == True and irregular == False:
                        aux_var = integrate.dblquad(f, i/N, (i+1)/N, 
                                                    lambda x: j/N, lambda x: (j+1)/N)
                        aux_var2 = aux_var[0]
                        if normalize == True:
                            aux_var2 = aux_var2 * N ** 2
                        
                    elif sampling == True and irregular == False:
                        #aux_var2 = np.random.binomial(1, f[i/N,j/N])
                        aux_var2 = f(i/N, j/N)

                    elif sampling == True and irregular == True:
                        aux_var2 = f(interval[i], interval[j])

                    elif integration == True and irregular == True:
                        if j == N-1 and i == N-1:
                            aux_var = integrate.dblquad(f, interval[i], 1, 
                                                    lambda x: interval[j], lambda x: 1)
                            if normalize == True:
                                aux_var = aux_var / (1-interval[i]) / (1-interval[j])

                        elif i == N-1:
                            aux_var = integrate.dblquad(f, interval[i], 1, 
                                                    lambda x: interval[j], interval[j+1])
                            if normalize == True:
                                aux_var = aux_var / (1-interval[i]) / (interval[j+1] - interval[j])

                        elif j == N-1:
                            aux_var = integrate.dblquad(f, interval[i], interval[i+1], 
                                                    lambda x: interval[j], 1)
                            if normalize == True:
                                aux_var = aux_var / (interval[i+1]-interval[i]) / (1-interval[j])

                        else:
                            aux_var = integrate.dblquad(f, interval[i], interval[i+1], 
                                                    lambda x: interval[j], lambda x: interval[j+1])
                            if normalize == True:
                                aux_var = aux_var / (interval[i+1] - interval[i]) / (interval[j+1]-interval[j])
            ...
            for i in range(N):
                for j in range(i,N):
                    ...
                    if integration == True:
                        if i < N-1:
                            endPointX = labels[i+1]
                        else:
                            endPointX = 1
                        if j < N-1:
                            endPointY = labels[j+1]
                        else:
                            endPointY = 1
                        
                        aux_var = integrate.dblquad(f, labels[i], endPointX, 
                                                  lambda x: labels[j], lambda x: endPointY)
                        if normalize == True:
                                aux_var[0] = aux_var[0] / (endPointX-labels[i]) / (endPointY-labels[j])

                        aux_var2 = aux_var[0]
                        
                    elif sampling == True:
                        #aux_var2 = np.random.binomial(1,f[labels[i],labels[j]])
                        aux_var2 = f(labels[i],labels[j])
...
class Graph():
    ...
    def __init__(self, graphType, N, graphOptions):
        ...
        if graphType == 'graphon':
            self.labels = graphOptions['labels']
            
            if self.labels == None:
                self.labels = np.arange(0, 1, 1/self.N)
