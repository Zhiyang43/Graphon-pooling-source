...
import Utils.interp1d as interp1d
...
class GraphonPoolLinearInterpolation(nn.Module):
    """

    """

    def __init__(self, inputLabels, outputLabels):

        super().__init__()
        self.inputLabels = torch.tensor(inputLabels).unsqueeze(0)
        self.outputLabels = torch.tensor(outputLabels).unsqueeze(0)
    ...
    def forward(self, x):
        assert x.shape[2] == self.inputLabels.shape[1]
        N1 = x.shape[2]
        F = x.shape[1]
        x = x.reshape(-1,N1)
        B1 = x.shape[0]
        y = torch.empty(B1,self.outputLabels.shape[1])
        f = interp1d.Interp1d(self.inputLabels.repeat(B1,1), x, self.outputLabels.repeat(B1,1),y)
        N2 = y.shape[1]
        return y.reshape(-1,F,N2)

    def extra_repr(self):
        reprString = "in_dim=%d, out_dim=%d " % (
                self.inputLabels.shape[0], self.outputLabels.shape[0])
        reprString += "no neighborhood needed"
        return reprString 
    
class GraphonPool(nn.Module):
    """
    ...
    def forward(self, x):
        ...
        assert x.shape[2] == len(self.inputLabels)
        B = x.shape[0]
        F = x.shape[1]
        y = torch.empty(B,F,len(self.outputLabels))
        for i in range(len(self.outputLabels)):
            for j in range(len(self.inputLabels)):
                if j == 0:
                    if (self.outputLabels[i] >= 0) and (self.outputLabels[i] < self.inputLabels[j]):
                        y[:,:,i] = x[:,:,j]
                elif j < len(self.inputLabels) - 1:
                     if self.outputLabels[i] >= self.inputLabels[j-1] and self.outputLabels[i] < self.inputLabels[j]:
                        y[:,:,i] = x[:,:,j]
                else:
                     if self.outputLabels[i] >= self.inputLabels[j-1] and self.outputLabels[i] <+ self.inputLabels[j]:
                        y[:,:,i] = x[:,:,j]
